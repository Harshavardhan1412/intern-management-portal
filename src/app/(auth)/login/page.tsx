...
function LoginContent() {
  ...
  useEffect(() => {
    if (roleParam && ['admin', 'director', 'intern'].includes(roleParam)) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (profile.role !== activeRole) {
      await supabase.auth.signOut()
      setError(`Access denied: These credentials belong to a user with the role "${profile.role}". Please select the correct tab to log in.`)
      setLoading(false)
      return
    }
    ...
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-500 font-medium">Loading login form...</p>
        </div>
      </div>
    }>
      <LoginContent />
